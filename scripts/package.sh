...
# `-xtype l` is a symlink whose target does not exist. Under `-L` those are otherwise reported as
# errors and, worse, would be stored as broken entries.
find -L . -type f -not -xtype l | LC_ALL=C sort | zip -X -q -@ "$archive"
